...
// Code taken from https://www.instructables.com/id/Arduino-String-Manipulation-Using-Minimal-Ram/
...
int basicInstr(const char *haystack, const char *needle);
int basicLen(const char *str1);
uint8_t basicAsc(const char *str1);
char* basicChr(uint8_t c);
char* basicHex(uint32_t n);
char* basicStr(long n);
long basicVal(const char *str1);
void basicValTest();
char* basicStringAdd(const char *str1, const char *str2);
...
int basicInstr(const char *haystack, const char *needle) // same as Basic instr
{
  ...
  if (index < 0) {
    // If strstr() doesn't find the *needle, return standard convention of -1
    return -1;
  }
  return index;
}
